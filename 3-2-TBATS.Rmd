...
```{r callsdata, message=FALSE, warning=FALSE, echo=FALSE}
p1 <- autoplot(calls) +
...
```{r callsmstl, fig.cap="Multiple STL for the call volume data.", fig.asp=.95, fig.height=6}
calls %>% mstl() %>%
  autoplot() + xlab("Week")
```
...
```{r callsmstlf, fig.cap="Multiple STL for the call volume data.", fig.asp=0.55}
calls %>%  stlf() %>%
  autoplot() + xlab("Week")
```
...
```{r callsharmonics, echo=TRUE, fig.cap="Forecasts from a dynamic harmonic regression applied to the call volume data.",fig.asp=0.55}
